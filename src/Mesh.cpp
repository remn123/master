		...
		while (!gmshFile.eof())
		{
			...
			if (_BLOCK_ == blocks::NODES)
			{
				...
				if (parser.size() > 2)
				{
					...
					this->nodes.emplace_back(Node{ newVec });
...
{
  std::cout << "Node(" << id << ").id = " << this->nodes[id].id << std::endl;
}
...
double Mesh::get_volume(const std::vector<Node>& vec)
{
  // pass
...
long Static_Mesh::mark_fringes(const Node& n2)
{
  int k = -1;
...
long Static_Mesh::_get_closest(const Node& n2, std::vector<long>& kneighbors)
{
...
long Static_Mesh::_find_element(const Node& n2, std::vector<long>& elm_ids)
{
	/*
	Inside the element:
	 _________
