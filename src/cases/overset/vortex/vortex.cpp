...
int main()
{
  ...
  background_msh->read_gmsh((cur_path.parent_path() / "resources" / "overset" / "vortex" / "lo" / "1" / "vortex_bkg.msh").string());
  nearbody_msh->read_gmsh((cur_path.parent_path() / "resources" / "overset" / "vortex" / "lo" / "1" / "vortex_bdy.msh").string());
  // Creating kd-tree
  ...
  */
  ...
  auto filename_bkgd = (cur_path.parent_path() / "results" / "output" / "overset" / "vortex"  / "ho" / "1" / "fringes" / "background.vtk" ).string();
  auto filename_body = (cur_path.parent_path() / "results" / "output" / "overset" / "vortex" / "ho" / "1" /  "fringes" / "near_body.vtk"  ).string();

  background_msh->to_vtk(filename_bkgd);
  nearbody_msh->to_vtk(filename_body);

  std::cout << "Saving Initial Condition ...\n";
  filename_bkgd = (cur_path.parent_path() / "results" / "output" / "overset" / "vortex"  / "ho" / "1" / "iterations" / "pp_vortex_bkgd_").string();
  filename_body = (cur_path.parent_path() / "results" / "output" / "overset" / "vortex"  / "ho" / "1" / "iterations" / "pp_vortex_body_").string();
